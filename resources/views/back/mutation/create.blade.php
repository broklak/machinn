...
@section('content')
    ...
    <div id="content-header">
        <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="{{route("$route_name.index")}}">@lang('module.mutation')</a> <a href="#" class="current">@lang('web.add') @lang('module.mutation')</a> </div>
        <h1>@lang('module.mutation')</h1>
    </div>
    <div class="container-fluid"><hr>
        <a class="btn btn-success" href="{{route('mutation.index')}}">@lang('web.view') Data</a>
            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title"> <span class="icon"> <i class="icon-pencil"></i> </span>
                        <h5>@lang('web.addButton') @lang('module.mutation')</h5>
                    </div>
                                    ...
                                    <div class="controls">
                                        <select name="from">
                                            ...
                                            @foreach($cash_account as $key => $val)
                                                <option @if(old('from') == $val['cash_account_id']) selected="selected" @endif value="{{$val['cash_account_id']}}">{{$val['cash_account_name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <select name="to">
                                            <option disabled selected>@lang('web.choose')</option>
                                            @foreach($cash_account as $key => $val)
                                                <option @if(old('to') == $val['cash_account_id']) selected="selected" @endif value="{{$val['cash_account_id']}}">{{$val['cash_account_name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
